...
proc DialogBox {args} {
  ...
  foreach {opt def} {title "DialogBox" text "" icon "" type ok \
                     ...
                     ent1focus 0 place center font TkDefaultFont} {
  ...
  foreach but $vartype {
    if {[lsearch $vartype $but]==$varaccpButIndx} {
      $dlg add -text $but -name $but -command [list EndDlg $dlg $but $varentQty]
    } else {
  ...
  if {$varentQty>0} {
    ...
    for {set fi 1} {$fi<$varentQty} {incr fi} {
      bind [set ent$fi] <Return> [list ReturnOnEntry [set ent$fi] $fi [list focus -force [set ent[expr {$fi+1}]] ]]
    }
    ## binding for the last Entry
    bind [set ent$varentQty] <Return> [list ReturnOnEntry [set ent$varentQty] $fi [list $dlg invoke $varaccpButIndx ] ]
  }
...
# EndDlg
# ***************************************************************************
proc EndDlg {dlg but varentQty} {
  global gaDBox
  set res 1
  for {set fi 1} {$fi<=$varentQty} {incr fi} {
    set res [ReturnOnEntry [$dlg getframe].fr.f$fi.ent$fi $fi [list return 1]]
    #puts "fi:$fi res:$res"
    if {$res!="1"} {return}
  }    
...
# ReturnOnEntry
# ***************************************************************************
proc ReturnOnEntry {e fi cmd} {
  global gaDBox
  set P [$e get]
  set res [EntryValidCmd $P]
...
# ***************************************************************************
proc EntryValidCmd {P} {
  set leng [string length $P]
	set rep [regexp -all { } $P]
	set num [string is alnum $P]
	if {($leng>0) && ($leng!=$rep) && ($num=="1")} {
	  return 1
  } else {
    return 0
  }
