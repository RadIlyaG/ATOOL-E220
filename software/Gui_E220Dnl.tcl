...
#     catch {exec taskkill.exe /im mb.exe /f /t}
    ...
    set ret 0
    GuiPower $gaSet(pair) 1 ; ## power ON before OpenRL
    set gaSet(plEn) 0
    if {$ret==0} {
       if {$ret==0} {
        IPRelay-Green
  ...
  if {$ret==0} {
    RLSound::Play pass
    if {[string match ETX-203AX_ATT.GE30.2SFP.2SFP.X.tcl $gaSet(DutInitName)]==1} {
      Status "Done $gaSet(serNum)"  #00ff00 ; #green
    } else {
...
proc GuiInventory {} {  
  ...
  if {![file exists uutInits/$gaSet(DutInitName)]} {
    ...
    foreach indx {Boot SW DGasp ExtClk Default 19 Half19 19SyncE Half19SyncE Aux1 Aux2 Conf} { 
...
#     pack $fr  -anchor w
  ...
  if 1 {
    ...
    foreach indx {SW Conf Default} {
      if {$indx==$gaSet(dutBox) || $indx=="DGasp" || $indx=="ExtClk" || \
          $indx=="${gaSet(dutBox)}SyncE" || $indx=="Aux1" || $indx=="Aux2" || \
          $indx=="Boot" || $indx=="SW" || $indx=="Default" || $indx=="Conf"} {
